...
describe("extractFacts", () => {
	it("captures Title Case prefix lines", () => {
		const facts = extractFacts(["Name: Ben", "Email: ben@codewalnut.com"]);
		expect(facts.get("name")).toBe("Ben");
	...
	it("ignores lines without a recognizable prefix", () => {
		const facts = extractFacts(["just a fact about Ben", "  : leading colon"]);
		expect(facts.size).toBe(0);
	});

	it("keeps the first occurrence on duplicate keys", () => {
		const facts = extractFacts(["Name: Ben", "Name: Other"]);
		expect(facts.get("name")).toBe("Ben");
	});
});

describe("auditFacts", () => {
	const lines = ["Name: Ben", "Email: ben@codewalnut.com", "Preferred address: Chief"];
	...
	it("matches case-insensitively and returns the value", () => {
		...
		expect(cells[0]).toEqual({ expected: "name", found: true, value: "Ben" });
		expect(cells[1]).toEqual({ expected: "EMAIL", found: true, value: "ben@codewalnut.com" });
	});
